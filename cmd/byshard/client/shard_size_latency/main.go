...
func main() {
	//
	// http.Get(fmt.Sprintf("http://127.0.0.1:%v/broadcast_tx_commit?tx=\"fromid=%v,toid=%v,type=%v,from=ABCD,to=EFGH,value=10,data=NONE,nonce=%v\"", 21057, 1, 1, 0, get_rand(math.MaxInt64)))
	fmt.Println("sending, current time is: " + time.Now().String())
	// send cross-shard tx
	resp, _ := http.Get(fmt.Sprintf("http://127.0.0.1:%v/broadcast_tx_commit?tx=\"fromid=%v,toid=%v,type=%v,from=EFGH,to=WXYZ,value=10,data=NONE,nonce=%v\"", 20057, 0, 1, 1, get_rand(math.MaxInt64)))

...
import (
	...
	byshardapp "github.com/AFukun/haechi/consensus/byshard/coordinator/abci"
...
func main() {
	...
	if isLeader == "true" {
		validatorInterface = byshardnode.NewValidatorInterface(db, uint8(shardNum), uint8(shardid), true, in_ip_temp, out_ips_temps)
	} else if isLeader == "false" {
		validatorInterface = byshardnode.NewValidatorInterface(db, uint8(shardNum), uint8(shardid), false, in_ip_temp, out_ips_temps)
	}

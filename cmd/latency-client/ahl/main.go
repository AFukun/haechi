...
func main() {
	...
	intra_req := fmt.Sprintf("http://%v:%v/broadcast_tx_commit?tx=\"fromid=%v,toid=%v,type=%v,from=EFGH,to=WXYZ,value=10,data=NONE,nonce=%v,txid=%v\"", shardIp, shardPort, 0, 0, 0, get_rand(math.MaxInt64), 1)
	...
	fmt.Println("AHL: cross-shard tx start in:", cross_start, "nonce is", nonce)
	http.Get(fmt.Sprintf("http://%v:%v/broadcast_tx_commit?tx=\"fromid=%v,toid=%v,type=%v,from=CROS,to=WXYZ,value=10,data=NONE,nonce=%v,txid=%v\"", beaconIp, beaconPort, 0, 1, 1, nonce, get_rand(20000)))
}
